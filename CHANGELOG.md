...
- Top-mount (TD40/TD200) sensors now correctly support a sensor mount height that differs from the max water level height, matching the two-value model the official Mopeka app uses ("Max Water Level Height" and "Overall Height"). Previously, any headspace caused the tank to never reach 100% full.

### 🎉 New Features!

- Added a "Sensor mount height" field for top-mount sensors, available in initial setup, reconfigure, and the options flow.

### 🛠️Changed

- Renamed the "Tank volume" sensor to "Tank volume remaining" for clarity.
- Standardized several sensor names to sentence case for consistency (`Battery Voltage` → `Battery voltage`, `Tank Level` → `Tank level`, `Signal Strength` → `Signal strength`).
- Bumped integration version to `0.2.8`.
